---
name: schedule-task-cron
description: Thiết lập lịch chạy tự động chuẩn cho Windows Task Scheduler và Linux cron, đặc biệt cho các dự án đăng bài Facebook/Fanpage hoặc automation chạy định kỳ.
---

# 🕒 Cross-Platform Schedule Setup

Skill này giúp thiết lập lịch chạy tự động chuẩn cho cả Windows và Linux, dùng được cho các dự án đăng bài Facebook/Fanpage hoặc automation chạy định kỳ.
...
```powershell
powershell -ExecutionPolicy Bypass -File ".agents\skills\schedule-task-cron\scripts\setup_schedule.ps1" -Mode "5Mins"
```
...
```powershell
powershell -c "Start-Process powershell -Verb runAs -ArgumentList '-ExecutionPolicy Bypass -File \"$(Get-Location)\.agents\skills\schedule-task-cron\scripts\setup_schedule.ps1\" -Mode \"Daily\" -Time \"09:00AM\"'"
```
