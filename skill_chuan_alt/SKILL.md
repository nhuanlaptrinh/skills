...
description: Kiểm tra và chuẩn hóa dự án theo chuẩn ALT, bao gồm cấu trúc project skill trong `.agents/skills`, đường dẫn tương đối, cấu hình .env, metadata chuẩn, venv và khử token/API/secret. Use when the user asks to kiểm tra skill, chuẩn hóa dự án, tạo skill chuẩn dự án, or hướng dẫn tạo skill dùng ở cấp dự án và global.
---
...
### 2. Cấu Trúc Skill Chuẩn Dự Án (`.agents/skills`)
- **Project skill bắt buộc đặt trong dự án theo cấu trúc**:
  ```text
  <thu-muc-du-an>/
    .agents/
      skills/
        <ten-skill>/
          SKILL.md
  ```
- **Tên thư mục skill** phải trùng với `name` trong frontmatter của `SKILL.md`. VD: `name: viet-hoa-thong-bao` thì thư mục phải là `.agents/skills/viet-hoa-thong-bao/`.
- **Không đặt project skill ở `.codex/skills` trong thư mục dự án**. `.codex/skills` chỉ dùng cho global/personal skills trong thư mục người dùng như `C:\Users\<user>\.codex\skills\`.
- **Skill tối thiểu cần có**:
  ```text
  .agents/
    skills/
      ten-skill/
        SKILL.md
  ```
- **Có thể thêm metadata giao diện nếu cần**:
  ```text
  .agents/
    skills/
      ten-skill/
        ...
        agents/
          openai.yaml
  ```
- **Cách tạo nhanh project skill**:
  1. Tạo thư mục `.agents/skills/<ten-skill>/`.
  2. Tạo file `.agents/skills/<ten-skill>/SKILL.md`.
  3. Viết frontmatter bắt buộc gồm `name` và `description`.
  4. Viết hướng dẫn ngắn gọn trong body: workflow, quy tắc output, ví dụ prompt.
  5. Validate lại skill nếu có tool validate.
- **Mẫu `SKILL.md` tối thiểu**:
  ```markdown
  ---
  name: ten-skill
  description: Mô tả rõ skill làm gì và khi nào cần dùng skill này.
  ---
  ...
  2. Làm theo quy tắc riêng của skill.
  3. Trả kết quả ngắn gọn, đúng ngữ cảnh.
  ```
- **Cách chuyển project skill thành global skill**: copy thư mục `<ten-skill>` từ `.agents/skills/<ten-skill>/` sang `C:\Users\<user>\.codex\skills\<ten-skill>\`.

### 3. Đường Dẫn Tương Đối (Relative Paths)
...
### 4. Tách Biệt Cấu Hình (.env) - Không Can Thiệp Code
...
### 5. Khử Token/API/Secret Trước Khi Đưa Lên GitHub - Bắt Buộc
...
### 6. Môi Trường Ảo Python (venv) - Không Bắt Buộc.
...
- **Lệnh chạy script** phải dùng python trong venv (không dùng `python` hệ thống):
  ```powershell
  .\venv\Scripts\python.exe .agents\skills\scripts\<TenScript>.py
  ```
...
2. **Kiểm tra cấu trúc project skill**:
   - Nếu dự án có skill nội bộ, skill phải nằm trong `.agents/skills/<ten-skill>/SKILL.md`.
   - Kiểm tra tên thư mục `<ten-skill>` có trùng với trường `name` trong frontmatter không.
   - Nếu thấy skill bị đặt nhầm trong `.codex/skills` ở thư mục dự án, chuyển về `.agents/skills`.
   - Nếu người dùng muốn dùng skill cho mọi dự án, hướng dẫn copy sang `C:\Users\<user>\.codex\skills\`.
3. **Tìm kiếm lỗi Hardcode**: Dùng công cụ (như `grep_search`) quét các file `.py` tìm `C:\`, `D:\`, `SPREADSHEET_ID = "..."` (hardcode string).
4. **Kiểm tra `.env`**: Xác nhận sự tồn tại của file `.env` và các biến môi trường cần thiết có được khai báo ở đó không.
5. **Khử token/API/secret trước khi GitHub**:
...
7. **Báo cáo và Đề xuất**: Liệt kê các lỗi vi phạm và đề xuất cách sửa (hoặc trực tiếp sửa bằng tool code edit nếu người dùng cho phép).
