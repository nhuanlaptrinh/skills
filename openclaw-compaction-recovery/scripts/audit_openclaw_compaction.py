...
PROFILE = {
    "reserveTokens": (24000, max),
    "reserveTokensFloor": (24000, max),
    "keepRecentTokens": (8000, min),
    "maxHistoryShare": (0.5, min),
...
def plan(config):
    ...
    agents = config.get("agents", {}).get("list", []) or [{"id": "main"}]
    blockers = []
    capacity = defaults.get("contextTokens")
    if not number(capacity) or capacity < 80000:
        blockers.append("A verified configured context cap of at least 80000 is required")
    if current.get("provider") or current.get("model"):
        blockers.append("Custom compaction provider/model requires individual review")
    if current.get("enabled") is False or current.get("mode") != "safeguard":
        blockers.append("Disabled or non-safeguard compaction requires individual review")
    if current.get("qualityGuard", {}).get("enabled") is False:
    ...
    for agent in agents:
        ...
        if number(agent.get("contextTokens")):
    ...
    if not model_windows or not number(capacity) or capacity < 80000:
        blockers.append("At least one agent/model is too small or unknown for the 24k baseline")
    ...
    if not number(retries) or retries < 0:
    ...
    reserves = [proposed.get(key, 0) for key in ("reserveTokens", "reserveTokensFloor")]
    if all(number(value) for value in reserves) and number(capacity) and max(reserves) > capacity / 3:
        blockers.append("Existing reserve would consume over one third of the smallest window")
    return candidate, {
...
def inspect_target(root):
    ...
    return report | {
        "root": str(root), "target": root.parent.name,
...
class Runtime:
    def __init__(self, root, container=None, runtime_root=None, binary="openclaw"):
        ...
        self.binary = binary
        if root.parent.parent == DATA_ROOT and not container:
...
def main(argv=None):
    ...
    try:
        ...
        roots = sorted(DATA_ROOT.glob("*/.openclaw")) if args.all_members else [args.openclaw_root or DATA_ROOT / args.member / ".openclaw"]
        ...
        for root in roots:
            try:
                if args.apply or args.rollback or args.validate_candidate:
                    inferred_container = args.container
                    inferred_runtime_root = args.runtime_root
                    if root.parent.parent == DATA_ROOT and not inferred_container:
                        account = root.parent.name.lower().replace("_", "-")
                        inferred_container = "user-" + account
                        inferred_runtime_root = Path("/home") / account / ".openclaw"
                    runtime = Runtime(root, inferred_container, inferred_runtime_root, args.openclaw_bin)
                if args.apply:
                ...
                reports.append(report | {"root": str(root), "target": root.parent.name})
