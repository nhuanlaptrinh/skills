...
description: Create or adapt an OpenClaw-backed course-advisor chatbot for Anh Lap Trinh websites, including multi-course portals and single-course landing pages. Use when Codex needs to add a chatbot before a registration form, register a restricted website agent with the shared OpenClaw AI Bridge, read course knowledge from /root/Data/second_brain/Second_Brain/01_chuong_trinh_dao_tao, optionally notify Telegram when requested, or reuse an existing advisor UI pattern.
---
...
- AI provider: shared OpenClaw AI Bridge. Do not add direct provider calls to individual websites.
- Provider fallback: configured centrally in the Bridge. Websites must not implement their own DeepSeek fallback or store fallback API keys.
- Secrets: `.env` only, never JS/HTML.
- Knowledge source: markdown files under the course folder in `Second_Brain`.

For OpenClaw-backed websites, read and follow the global skill `openclaw-course-website-bridge`. Keep the frontend contract `/api/course-advisor/`, create a restricted agent per website, and never mount or copy the OpenClaw gateway token into a website container.
...
4. Build the backend endpoint.
   - Read and follow `openclaw-course-website-bridge` before implementing the backend.
   - Register one restricted agent `website-<project>` and one Bridge allowlist key per website.
   - Use `OPENCLAW_BRIDGE_URL`, `OPENCLAW_BRIDGE_PROJECT`, and `OPENCLAW_TIMEOUT_SECONDS`; never place the OpenClaw gateway token in the website project.
   - Send `project`, a session identifier, and `messages` to the Bridge endpoint.
   - Always fall back from `message.content` to `message.reasoning_content` before cleaning the answer. Never assume `content` is non-empty.
   ...
   - Separate generic tuition intent from specific operating-cost intent. If a customer asks only about `chi phí`, `giá`, or `học phí`, answer the course tuition, VAT/total, and learning support such as `học vướng đâu gỡ đó`; do not volunteer API/token, VPS/server, software, maintenance, service, or other-course costs unless the customer explicitly asks about that category.
...
5. Build the frontend.
   ...
   - Show a visible loading state while OpenClaw answers.
...
6. Optional Telegram notification.
   ...
   - Telegram remains a website-side notification after a successful OpenClaw answer; never send real notifications during tests.
...
7. Configure deployment.
   ...
   - Add only Bridge URL, project key, and timeout to `.env.example`; no AI provider secret belongs in a website `.env`.
   - Confirm the website container can reach `GET http://172.18.0.1:18890/health`.
   - Recreate/restart the running service after adding `env_file` or volumes, e.g. `docker compose up -d --force-recreate <service>`.

8. Verify.
   - After every manual or bulk Python edit, run `python3 -m py_compile` on every changed backend file before relying on Django autoreload. Bulk insertion can easily leave a closing brace or response-reading block at the wrong indentation.
   - Run framework checks, e.g. `python3 manage.py check`.
   - For Docker projects, also run `docker exec <container> python manage.py check` to confirm the running container can import the changed code.
   ...
   - Confirm the API returns `configured: true` and a non-empty `answer`; HTTP 200 alone is not enough.
   - If the API returns `configured: false`, inspect Bridge URL/project configuration and confirm the project exists in `/root/AI_Runtime/openclaw_bridge/config/projects.json`.
...
- Never put the OpenClaw gateway token or any real API key in website JS, HTML, templates, `.env.example`, committed files, or skill files.
- Keep real `.env` local and ignored.
- If the user pasted a real key in chat, warn that rotating the key is best practice.

## References

- Shared runtime and migration workflow: `openclaw-course-website-bridge`.
