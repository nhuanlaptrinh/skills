...
For models that may enable Thinking Mode, including `deepseek-v4-flash`, explicitly disable it for fast website advice and retain a fallback to `reasoning_content`. Some DeepSeek responses can leave `message.content` empty while putting the usable answer in `message.reasoning_content`.
...
response = requests.post(
    ...
    json={
        ...
        'messages': [
            {'role': 'system', 'content': 'Bạn tư vấn khóa học Anh Lập Trình theo dữ liệu nội bộ được cung cấp.'},
...
raw_answer = message_data.get('content') or message_data.get('reasoning_content') or ''
answer = _clean_chatbot_answer(raw_answer)
```
...
Treat both an empty key and `Nhap_API_Cua_Ban` as unconfigured. Check runtime configuration without printing the secret:

```bash
docker exec <service-name> sh -lc '
v=${DEEPSEEK_API_KEY:-}
if [ -z "$v" ]; then echo MISSING
elif [ "$v" = Nhap_API_Cua_Ban ]; then echo PLACEHOLDER
else echo PRESENT
fi'
```
...
After editing one or many chatbot backends:

```bash
python3 -m py_compile path/to/course_advisor.py
docker exec <service-name> python manage.py check
docker compose config --quiet
```

Then call the public endpoint and verify all three conditions:

- response is valid JSON
- `configured` is `true`
- `answer` is a non-empty string

Do not treat HTTP 200 or Django autoreload alone as proof that the chatbot works. If many similar projects were updated, compile and test every changed backend because bulk edits can introduce indentation errors around the request JSON closing brace or the `choices/message_data/raw_answer` block.
