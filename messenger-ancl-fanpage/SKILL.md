# messenger-ancl-fanpage

Use when operating, checking, starting, or updating the Facebook Messenger auto-reply bot for the ANCL course Fanpage.

## Project

- Project path: `/root/Automation/facebook/01_Mess_Fanpage/01_mes_op_ancl`
- App package: `app/`
- Local port: `8813`
- Local health check: `http://127.0.0.1:8813/health`
- Knowledge root: `/root/Data/second_brain/Second_Brain/01_chuong_trinh_dao_tao/19_domain_ancl`
- Course website: `https://ancl.anhlaptrinh.vn/`
- SQLite DB: `/root/Automation/facebook/01_Mess_Fanpage/01_mes_op_ancl/data/conversations.db`
- Primary AI backend: OpenClaw Chat Completions at `http://127.0.0.1:18789/v1/chat/completions`
- Secondary AI backend: DeepSeek Chat Completions from the ANCL website environment
- OpenClaw agent: `openclaw/messenger-ancl`
- OpenClaw model: `9r/GPT-5.6-luna`
- OpenClaw workspace: `/root/.openclaw/workspace_messenger_ancl`
...
- Start the service only after `META_PAGE_ID` and `META_PAGE_ACCESS_TOKEN` are confirmed to belong to the ANCL Fanpage.
...
- Keep the OpenClaw gateway on loopback; do not expose its operator bearer token publicly or copy it into project documentation.

## Dry Run / Local Checks

```bash
cd /root/Automation/facebook/01_Mess_Fanpage/01_mes_op_ancl
...
```bash
cd /root/Automation/facebook/01_Mess_Fanpage/01_mes_op_ancl
/usr/bin/python3 -m uvicorn app.main:app --host 127.0.0.1 --port 8813
```

Health check:

```bash
curl http://127.0.0.1:8813/health
```
...
```bash
/etc/systemd/system/01_mes_op_ancl.service
```

Start only after ANCL Fanpage credentials are set and approved:

```bash
systemctl start 01_mes_op_ancl.service
systemctl status 01_mes_op_ancl.service --no-pager -l
```

Enable auto-start only after production verification:

```bash
systemctl enable 01_mes_op_ancl.service
```
...
```text
https://synalt.anhlaptrinh.vn/messenger-ancl/webhook/facebook
```

Proxy target:

```text
http://127.0.0.1:8813/webhook/facebook
```

## Inputs

- Meta webhook events from the ANCL Fanpage.
...
- ANCL knowledge Markdown/TXT files under the knowledge root.
- DeepSeek configuration from `/root/Apps/course_websites/10Web_BH/19_domain_ancl/.env` if present.
...
- Service logs in `journalctl -u 01_mes_op_ancl.service` after systemd start.

## Human Takeover Behavior

- Configure `META_APP_ID` when available so the webhook can ignore bot-origin echoes by app ID.
- Bot Send API messages include metadata `01_mes_op_ancl:auto_reply`; echoes with this metadata are ignored.
...
- `app/chatbot.py` first calls model `openclaw/messenger-ancl`.
- Each Messenger PSID is SHA-256 hashed before being used as the OpenClaw `user` session key.
- OpenClaw agent `messenger-ancl` uses model `9r/GPT-5.6-luna`, workspace isolation, tool profile `minimal`, and denies runtime/filesystem/browser/messaging tools.
