---
name: selenium-zalo-list-relative
description: Tự động hóa nhắn tin Zalo Web theo tên - tìm kiếm danh sách cá nhân, gửi văn bản + hình ảnh từ Google Sheet.
---
...
Mở Terminal tại **thư mục gốc của dự án hiện hành** và chạy các lệnh sau:
```powershell
# Tạo môi trường ảo
python -m venv venv
...
- **Credentials**: File `gen-lang-client-*.json` phải nằm ở thư mục gốc dự án.
- **Ảnh**: Các file ảnh phải nằm trong thư mục `images/` ở **thư mục gốc dự án** (ví dụ: `images/hoc-python.png`).
- **Profile**: Thư mục `zalo-chrome-profile/` nằm trong `.agent/skills/scripts/`, tự động lưu phiên đăng nhập Zalo sau lần quét QR đầu tiên.
...
```powershell
.\venv\Scripts\python.exe .agent\skills\scripts\OpenZaloSendListRelative.py
```
...
- **Root Script**: File thực thi chính nằm tại `.agent/skills/scripts/OpenZaloSendListRelative.py`.
...
- **Image Sending**: Ảnh được đọc từ `images/` ở thư mục gốc dự án. Sử dụng PowerShell để bơm ảnh vào Clipboard và `Ctrl+V` để dán vào Zalo.
...
> [!IMPORTANT]
> **Lệnh chạy nhanh**: `.\venv\Scripts\python.exe .agent\skills\scripts\OpenZaloSendListRelative.py`
