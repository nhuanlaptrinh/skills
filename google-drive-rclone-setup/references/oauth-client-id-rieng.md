# OAuth Client ID riêng + quyền người dùng cho Google Drive rclone
...
```bash
install -d -m 700 /root/Data/private_accounts/google_drive
umask 077
nano /root/Data/private_accounts/google_drive/gdrive_oauth.env
```
...
```bash
chmod 600 /root/Data/private_accounts/google_drive/gdrive_oauth.env
```
...
install -m 600 "$CONFIG_FILE" \
  "/root/_Backups/rclone/rclone.conf.$(date -u +%Y%m%dT%H%M%SZ).before-oauth-private.bak"
```
...
```bash
set -a
. /root/Data/private_accounts/google_drive/gdrive_oauth.env
set +a
...
```bash
umask 077
nano /root/Data/private_accounts/google_drive/gdrive_oauth_user.json
chmod 600 /root/Data/private_accounts/google_drive/gdrive_oauth_user.json
```
...
TOKEN_VALUE="$(cat /root/Data/private_accounts/google_drive/gdrive_oauth_user.json)"
...
- Cập nhật `/root/_Second_AI_Brain/06_Nhat_Ky_Thay_Doi.md` bằng thông tin đã redacted.
