...
# Cấu hình mặc định
DEFAULT_CREDENTIAL_FILE = Path("/root/.cloudflare_subdomain_token")
DEFAULT_IP = "187.127.177.163"
...
def load_api_token() -> str:
    ...
    if not token:
        raise RuntimeError(
            "Thiếu Cloudflare API token. Cấu hình CLOUDFLARE_API_TOKEN hoặc "
            f"{DEFAULT_CREDENTIAL_FILE}."
        )
    return token
...
class CloudflareDNS:
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.api_key = api_key or load_api_token()
        self.headers = {
    ...
    def create_a_record(self, subdomain: str, ip: str = DEFAULT_IP, 
        ...
        Args:
            ...
            ip: Địa chỉ IP (mặc định: 187.127.177.163)
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Ví dụ: python cloudflare_dns.py vps7.anhlaptrinh.vn 187.127.177.163")
        sys.exit(1)
