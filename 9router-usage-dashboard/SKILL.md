...
- Keep every existing code in `settings.TOKEN_PROMOTIONS` working. A custom coupon cannot reuse a legacy code.
- Never delete a coupon after it has appeared on a `TokenPurchase`; the management page deactivates it instead so payment and redemption history remain auditable.
