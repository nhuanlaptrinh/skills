...
The public route `/` presents Token Codex pricing and benefits before authentication. A red/gold holiday offer strip immediately below navigation must emphasize `ƯU ĐÃI ĐẶC BIỆT NHÂN DỊP LỄ – Mua 1 Credit nhận 20 Credits`, state that the standard ×20 rate is automatic for new purchases without a promotion code, and link authenticated users to `/mua-token/` or anonymous visitors to `/dang-ky/`. Keep the responsive styles in `static/css/app.css` and update the landing stylesheet cache-busting query whenever the landing CSS changes. All customer-facing commercial copy uses Credits, including `1 Credit paid = 20 Credits provider-equivalent usage credit`; do not display USD labels in package selection, previews, promotion feedback, purchase history, or QR order details. Historical internal field and setting names ending in `_usd` remain unchanged for database and webhook compatibility. The page shows the corresponding 95% saving and concrete VND examples, explains that Credits are internal usage units rather than cash or a provider-sold account, and offers separate registration and login actions. Use the Anh Lập Trình logo at `static/img/logo-anh-lap-trinh.png`. The authenticated dashboard route is `/bang-dieu-khien/`.

The public route `/huong-dan-tich-hop/` documents the OpenAI-compatible Base URL `https://codex.anhlaptrinh.vn/v1`. Keep examples for OpenClaw, ChatGPT Codex in Antigravity, NousResearch Hermes Agent, Python, and Node.js. Token Codex currently accepts the exact model IDs `GPT-6-sol`, `GPT-6-terra`, `GPT-6-luna`, and `GPT-6-astra`. Use SOL as the default in short code examples, list all four in the guide and OpenClaw provider configuration, and remind users that model IDs are case-sensitive. Examples must use placeholders or environment-variable names rather than real keys. Keep backward compatibility for existing requests using `GPT-5.6-sol`, `GPT-5.6-terra`, and `GPT-5.6-luna`; the four `GPT-6-*` IDs are the current recommended list. Users may query `/v1/models` with their own key to confirm the current list, and must not append `/v1` twice. Keep links to this guide on both the public landing page and authenticated dashboard.
...
Customer-facing usage cost for provider `deepseek` uses `DEEPSEEK_CUSTOMER_COST_MULTIPLIER` (production `80`) from the UTC timestamp configured by `DEEPSEEK_CUSTOMER_COST_EFFECTIVE_AT` (`2026-09-10T07:36:07Z`); other providers stay unchanged. When a prior approved rate window exists, configure `DEEPSEEK_CUSTOMER_COST_PREVIOUS_MULTIPLIER` and `DEEPSEEK_CUSTOMER_COST_PREVIOUS_EFFECTIVE_AT` so records in that window retain the earlier multiplier (`50` from `2026-09-10T06:47:24Z`). Records before the prior window remain raw and are never re-rated. Keep the source `usageHistory.cost` unchanged and route dashboard totals, request activity, member reports, admin/Telegram usage, key close-out totals, and credit enforcement through the shared customer-cost helper.

The authenticated dashboard also includes a paginated request activity ledger using the same date filter. Each row may show the internal usage row ID, Vietnam timestamp to the second, assigned API name, provider/model, endpoint, status, prompt/completion token counts, and USD cost. Keep it at 50 rows per page. On the customer dashboard only, mask the upstream DeepSeek identity in the presentation layer: provider `deepseek` displays as `alt`, and a leading model prefix such as `deepseek-v4-flash` displays as `alt-v4-flash`. Keep raw provider/model values for superuser/admin history and all billing, source, and audit logic. Never expose the complete API key in usage reports, connection ID, prompt, response, `meta`, or other request content.
...
Customers can buy additional Token Codex credit at `/mua-token/`. Customer-facing packages are labeled from 10 Credits through 1000 Credits in 10-Credit steps; arbitrary values are rejected. The displayed payment rate is `1 Credit = 25,000 VND`, and each standard purchased Credit adds 20 Credits to the existing `CustomerAccount.credit_limit`. Example: a standard 10-Credit package costs 250,000 VND and adds 200 Credits. Keep every visible payment label in Credits across package selection, preview values, promotion feedback, purchase history, QR details, Telegram, and referral-wallet copy. Internal schema and variables such as `purchase_usd`, `provider_credit_usd`, `TOKEN_PAYMENT_VND_PER_USD`, and related promotion keys stay unchanged for backward compatibility. `TOKEN_PAYMENT_PROVIDER_MULTIPLIER` is authoritative for standard website, Telegram, and referral-wallet purchases; keep the production `.env`, settings fallback, visible copy, preview values, and tests aligned. Each order locks its credit values when created, so multiplier changes affect only new orders and must not recalculate pending, paid, expired, or historical `TokenPurchase` rows.

The purchase page shows only a neutral promotion-code input and does not display active codes in its visible copy. Standard ×20 credit is automatic and does not require a coupon. `CHAOMUNG30` adds 30% bonus credit for a customer's first paid purchase. `THANTHIET50` adds 50% bonus credit and may be used once per customer even if they have previous purchases. `THANTHIETX20` and `THANTHIET15` remain compatibility coupons that lock the same ×20 multiplier as the standard rate while preserving their historical redemption rules; they do not add credit above the standard rate. `DNX25` applies a provider-credit multiplier of 25 to new purchases of 40 USD or more, preserves the package's normal VND charge, and is repeatable without a redemption limit: 40 USD adds 1,000 USD credit and 100 USD adds 2,500 USD. Do not recalculate already-created orders or already-granted credit when changing standard or coupon multipliers. `HOCVIENKH` is a free student trial coupon: it represents a 2 USD customer value, immediately adds 20 USD provider credit without VietQR payment, and may be redeemed only once per customer. `NGUOITHAN400` is a private family coupon: it represents a 40 USD customer value, immediately adds 400 USD provider credit without payment, and may be redeemed only once per customer. Redeem free-credit coupons inside a database transaction while locking `CustomerAccount`, and record a zero-VND `paid` `TokenPurchase` so repeat use is rejected. Percentage promotions keep the VietQR amount unchanged, apply from the 10 USD package, and cap the bonus at 1,000 USD. Repeatable coupons `DAMUA3000K` and `DAMUA4000K` override the selected package with a 3,000 VND or 4,000 VND payment respectively and exactly 100 USD provider credit; customers may use repeatable coupons again after the previous order with that code is paid or expired. Store the normalized code and locked amount/credit values on `TokenPurchase`; credit `provider_credit_usd + promotion_bonus_usd` only after full payment. Reject unknown codes, purchases below a promotion-specific `min_purchase_usd`, redemption beyond a configured `max_redemptions`, reuse of non-repeatable paid codes, and a second active order using the same code. Configuration uses `TOKEN_PROMOTIONS`, `TOKEN_PROMOTION_MIN_PURCHASE_USD`, `TOKEN_PROMOTION_THANTHIETX20_MIN_PURCHASE_USD`, `TOKEN_PROMOTION_DNX25_MIN_PURCHASE_USD`, and `TOKEN_PROMOTION_MAX_BONUS_USD` without requiring secrets.
...
New website and Telegram registrations require a valid six-character referral code. Every `CustomerAccount` has a unique code derived from the email local part where possible. After a SePay order becomes `paid`, credit the referrer with `REFERRAL_COMMISSION_PERCENT` (default 30%) of the order's actual VND charge for at most `REFERRAL_COMMISSION_MAX_PURCHASES` (default 3) qualifying purchases per referred customer, including fixed-price coupons such as `DAMUA3000K` and `DAMUA4000K`. Record the commission and wallet ledger entry in the same database transaction; never credit again for the same `TokenPurchase`, and never generate commission for referral-wallet purchases. The separate VNĐ wallet can buy standard Token packages at the normal ×20 rate or request a bank withdrawal from `REFERRAL_WITHDRAWAL_MIN_VND` (default 500,000 VNĐ). Withdrawal requests reserve balance immediately; admin actions mark them paid or reject and refund through `/admin/`. Telegram exposes `Giới thiệu & Ví hoa hồng`, `/gioithieu`, wallet purchase, and withdrawal flows. Never log full bank account details, secrets, tokens, API keys, prompts, or responses.
